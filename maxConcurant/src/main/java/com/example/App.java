...
public class App implements Runnable {
    @Option(names = { "--intervals" }, description = "Space-separated intervals: '1,5 2,7 3,8'", required = false)
    ...
    private int[][] parseIntervals(String input) {
        // Split by spaces to get individual interval pairs
        String[] pairs = input.trim().split("\\s+");
    ...
    private static int[][] maxConcurant(int[][] intervals, int max) {
        ...
        // Sort events by time, prioritizing start events over end events at the same
        // time
